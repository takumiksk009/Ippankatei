...
REM update_ippankatei_safe.bat  (ASCII/CRLF)
REM Run INSIDE the ippankatei app folder.
REM Safe push: keeps history (no force).

echo ===== UPDATE ippankatei (safe) =====
...
REM Always point origin to ippankatei
...
git add -A
git commit -m "chore: update ippankatei" || echo (No changes to commit)
